...
//__device__ __managed__ u32 sth;
...
__device__ __managed__ u32 st[5*1024];
__device__ __managed__ int count=0;
...
__global__ void mykernel(int input_size) {
  ...
  vm_init(&vm, data, storage, pt, &pagefault_num, PAGE_SIZE,
          ...
          PHYSICAL_MEM_SIZE / PAGE_SIZE,st);
  
  vm.id = threadIdx.x;
  if(0 == vm.id){
    for(int cc=0;cc<4*1024;cc++){
      st[cc] = 0x80000000;
    }
    for (int i = 0; i < 1024; i++) {
      vm.invert_page_table[i] = 0x80000000; // invalid := MSB is 1
      vm.invert_page_table[i + 1024] = 0;
    }
    user_program(&vm, input, results, input_size);
    //count += *vm.pagefault_num_ptr;
    //user_program(&vm, input, results, input_size);
  }
  __syncthreads();
  if(1 == vm.id){
